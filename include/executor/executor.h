...
public:
  ...
  const GC::Allocator &getAllocator() const noexcept { return Allocator; }
  ...
  ///
  /// convValsToComponent wraps scalar results (i32/i64/f64/...) in the same
  /// ValVariant alternative as references, so holding a ValVariant is not
  /// enough to tell a managed reference from a scalar. Reinterpreting a
  /// scalar's bits as a managed pointer could, by pointer-identity collision,
  /// drop an unrelated retained root. Release only when the embedded reference
  /// is a retained struct/array root, matching the C API's isRetainedRefValue
  /// guard.
  ///
  /// This relies on two invariants that keep a scalar's bits from masquerading
  /// as a managed pointer: (1) cleanNumericVal zero-extends scalar results, so
  /// the pointer word of a scalar payload is all-zero and getType() reports a
  /// non-ref type (filtered out below); (2) flatSize excludes v128 from
  /// component lifting, so no 128-bit scalar fills both words. Extending
  /// component lifting to v128 would break this -- reject 128-bit payloads here
  /// when that lands.
...
public:
  ...
  Expect<void *> proxyFuncGetFuncSymbol(Runtime::StackManager &StackMgr,
                                        const uint32_t FuncIdx) noexcept;
  Expect<void> proxyGlobalSet(Runtime::StackManager &StackMgr,
